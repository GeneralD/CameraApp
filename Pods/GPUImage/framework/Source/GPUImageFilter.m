...
- (CGImageRef)newCGImageFromCurrentlyProcessedOutputWithOrientation:(UIImageOrientation)imageOrientation
{
    ...
    NSAssert(self.outputTextureOptions.internalFormat == GL_RGBA, @"For conversion to a CGImage the filter texture format for this filter must be GL_RGBA.");
    NSAssert(self.outputTextureOptions.type == GL_UNSIGNED_BYTE, @"For conversion to a CGImage the type of the filter texture of this filter must be GL_UNSIGNED_BYTE.");
...
{
//    NSLog(@"Cleaning up filter filter image: %@", self);
    [self destroyFilterFBO];
    [self deleteOutputTexture];
}
...
{
    // I'm temporarily disabling adjustments for smaller filter sizes until I figure out how to make this work better
    return CGSizeZero;
